...
class ComplEx(torch.nn.Module):
    def __init__(self, d, d1, d2, **kwargs):
        ...
        self.Er = RandomEmbedding(len(d.entities), d1)
        self.Rr = RandomEmbedding(len(d.relations), d2)
        self.Ei = RandomEmbedding(len(d.entities), d1)
        self.Ri = RandomEmbedding(len(d.relations), d2)
    ...
    def init(self):
        nn.init.xavier_normal_(self.Er.weight.data)
        nn.init.xavier_normal_(self.Rr.weight.data)
        nn.init.xavier_normal_(self.Ei.weight.data)
        nn.init.xavier_normal_(self.Ri.weight.data)
...
class ConvE(torch.nn.Module):
    def __init__(self, d, d1, d2, **kwargs):
        ...
        self.register_parameter("b", nn.Parameter(torch.zeros(len(d.entities))))
...
class LinearHypER(torch.nn.Module):
    def __init__(self, d, d1, d2, **kwargs):
        super(LinearHypER, self).__init__()
        self.in_channels = kwargs["in_channels"]
        self.out_channels = kwargs["out_channels"]
        self.filt_h = kwargs["filt_h"]
        self.filt_w = kwargs["filt_w"]
        ...
        self.E = torch.nn.Embedding(len(d.entities), d1, padding_idx=0)
        self.R = torch.nn.Embedding(len(d.relations), d2, padding_idx=0)
        self.re_wr = torch.nn.Embedding(
            len(d.relations), d2 // 2, max_norm=d2, norm_type=1
        )
        self.im_wr = torch.nn.Embedding(
            len(d.relations), d2 // 2, max_norm=d2, norm_type=1
        )
    ...
    def init(self):
        nn.init.xavier_normal_(self.E.weight.data)
        nn.init.xavier_normal_(self.R.weight.data)
        nn.init.xavier_normal_(self.re_wr.weight.data)
        nn.init.xavier_normal_(self.im_wr.weight.data)

    def forward(self, e1_idx, r_idx):
        e1 = self.E(e1_idx)
        e1r, e1i = torch.chunk(e1, 2, dim=-1)
        re_wr = self.re_wr(r_idx)
        im_wr = self.im_wr(r_idx)
        e1r = re_wr * e1r
        e1i = im_wr * e1i
        e1 = torch.cat((e1r, e1i), dim=1).view(-1, 1, 1, self.E.weight.size(1))
        ...
        k = k.view(
            e1.size(0) * self.in_channels * self.out_channels,
            1,
            self.filt_h,
            self.filt_w,
        )
...
class HypE(torch.nn.Module):
    ...
    def forward(self, e1_idx, r_idx):
        ...
        x = F.conv2d(x, k, groups=e1.size(0))
        x = x.view(
            e1.size(0), 1, self.out_channels, 10 - self.filt_h + 1, 20 - self.filt_w + 1
        )
        x = x.permute(0, 3, 4, 1, 2)
        x = torch.sum(x, dim=3)
        x = x.permute(0, 3, 1, 2).contiguous()
